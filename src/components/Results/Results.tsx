			...
			try {
				...
				setConstructorStandings(
					data.MRData.StandingsTable.StandingsLists[0].ConstructorStandings
						.sort((a: ConstructorStanding, b: ConstructorStanding) => parseInt(b.points) - parseInt(a.points)) // Sort by points descending
				);
			} catch (err) {
				...
				<div className="results__content">
					<div className="content__drivers">
						...
						<table className="content__driversTable">
                            ...
                            <thead>
                                <tr>
                                    <th style={{width: 62, height: 42}}>No</th>
                                    <th style={{width: 98}}>Team</th>
                                    <th style={{width: 350}}>Name</th>
                                    <th style={{width: 82, paddingLeft: "8px"}}>Pts</th>
                                </tr>
                            </thead>
                            <tbody>
                                {top10Drivers.map((driver) => (
                                    <tr key={driver.position} className="driver__info">
                                        <td style={{width: 62, height: 42, paddingLeft: "8px"}}>{driver.position}</td>
                                        <td style={{width: 98}}>
                                            <img
                                        ...
                                        <td style={{width: 350, paddingLeft: "8px"}}>
                                            {`${driver.Driver.givenName} ${driver.Driver.familyName}`}
                                        </td>
                                        <td style={{width: 82, paddingLeft: "8px"}}>{driver.points}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
					...
					<div className="content__drivers">
						...
						<table className="content__driversTable">
								...
								<thead>
									<tr>
										<th style={{width: 62, height: 42}}>No</th>
										<th style={{width: 456, height: 42, paddingLeft: "8px"}}>Name</th>
										<th style={{width: 82, height: 42, paddingLeft: "8px"}}>Pts</th>
									</tr>
								</thead>
								<tbody>
									{constructorStandings.map((constructor) => (
										<tr key={constructor.position} className="driver__info">
											<td style={{width: 62, height: 42, paddingLeft: "8px"}}>{constructor.position}</td>
											<td style={{width: 456, height: 42, paddingLeft: "8px"}}>
											{`${constructor.Constructor.name}`}
											</td>
											<td style={{width: 82, height: 42, paddingLeft: "8px"}}>{constructor.points}</td>
										</tr>
									))}
								</tbody>
							</table>
